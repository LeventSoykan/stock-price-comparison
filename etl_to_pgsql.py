...
if __name__ == '__main__':
    ...
    # Load income_statement data to Postgres DB
    etl.merge(etl.get_info).to_sql('stock_info', engine, schema='public', index=False, if_exists='replace')
    time.sleep(65)
    ...
    # Load income_statement data to Postgres DB
    etl.merge(etl.get_financial, 'income_statement').to_sql('income_statement', engine, schema='public', index=False,
                                                            if_exists='replace')
    time.sleep(65)

    # Load balance sheet data to Postgres DB
    etl.merge(etl.get_financial, 'balance_sheet').to_sql('balance_sheet', engine, schema='public', index=False,
                                                         if_exists='replace')
